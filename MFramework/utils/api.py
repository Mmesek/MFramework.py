...
class Steam:
    ...
    async def getAppList(self):
        return await self.api_call(
            "IStoreService/GetAppList/v1/", f"?key={self.token}&have_description_language=english"
        )
...
class Twitter:
    ...
    async def twitter_call(self, path, querry, method="GET", **kwargs):
        ...
        kwargs = dict(defaults, **kwargs)
        response = await self.ClientSession.request(
            method, "https://api.twitter.com/1.1/" + path + "?" + querry, **kwargs
        )
        try:
            return await response.json()
        except:
    ...
    async def user_timeline(self, db, user):
        ...
        if tweet[0]["id_str"] != last_tweet_id:
            embed = (
                ...
                .setAuthor(
                    tweet[0]["user"]["name"],
                    f"https://twitter.com/{tweet[0]['user']['screen_name']}",
