...
from MFramework import Snowflake, Guild_Member, User, Embed
...
class Leaderboard_Entry:
    ...
    user_id: Snowflake
    _value: int
    def __init__(self, ctx: Context, user_id: Snowflake, value: int, value_processing: Callable = lambda x: x) -> None:
        self.ctx = ctx
...
class Leaderboard:
    ...
    def as_embed(self, title: str = "Leaderboard", add_user: bool = True, user_title: str = None, user_inline: bool = False) -> Embed:
        ...
        if add_user and self.user_stats and not self._user_found:
            if not user_title:
                user_title = "Your Stats" if not self.user_id or self.ctx.user.id == self.user_id else f"{self._user_stats.name}'s Stats"
