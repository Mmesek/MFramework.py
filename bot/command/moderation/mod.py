        ...
        if g not in {"Global", "Vip", "Nitro"}:
            ...
            if infraction[0] == "Mute":
                try:
                    ...
                    r = await self.endpoints.role_add(
...
async def serverInfo(self, *args, data, **kwargs):
    ...
    embed = (
        ...
        .setTimestamp(str(await created(server["id"])))
                ...
                embed = (
                    ...
                    .addField(
                        ...
                        str(
                            time.strftime(
                                "%Y-%m-%d %H:%M:%S",
                                datetime.datetime.fromisoformat(str(await created(role["id"]))).timetuple(),
                            )
                        ),
                        True,
                    )
