...
class Cache:
    ...
    def message(self, data):
        if len(self.cache[data['guild_id']]['msgs']) == 256:
            self.cache[data['guild_id']]['msgs'].pop(
                list(self.cache[data['guild_id']]['msgs'].keys())[0], None)
        ...
        if db is None:
            ...
            for channel in data['channels']:
                if 'bot' in channel['name']:
                    cnoexp += [channel['id']]
            await self.db.insert('Servers', 'GuildID, AdminIDs, ModIDs, VipIDs, NitroIDs, MutedIDs, NoExpRoles, NoExpChannels',
                                 [data['id'], json.dumps(admin), json.dumps(mod), json.dumps(vip), json.dumps(nitro), json.dumps(muted), json.dumps(rnoexp), json.dumps(cnoexp)])
            db = await self.db.selectOne('Servers', 'AdminIDs, ModIDs, VipIDs, NitroIDs, MutedIDs, NoExpRoles, NoExpChannels', 'WHERE GuildID=?', [data['id']])
